    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow-md p-8">
                ...
                <form action="{{ route('questions.store', $chapter) }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="question_text" required rows="3"
                                  ...
                                  placeholder="Tulis soal Anda...">{{ old('question_text') }}</textarea>
                        @error('question_text')
                            ...
                            <div id="choicesContainer" class="space-y-3">
                                <div class="flex gap-2">
                                    <input type="text" name="choices[]" class="flex-1 border border-gray-300 rounded-lg px-4 py-2"
                                           placeholder="Pilihan 1" value="{{ old('choices.0') }}">
                                    <input type="radio" name="correct_answer" value="" class="mt-3" {{ old('correct_answer') === (old('choices.0') ?? '') ? 'checked' : '' }}>
                                    <span class="text-sm text-gray-600 mt-3">Benar</span>
                                </div>
                                <div class="flex gap-2">
                                    <input type="text" name="choices[]" class="flex-1 border border-gray-300 rounded-lg px-4 py-2"
                                           placeholder="Pilihan 2" value="{{ old('choices.1') }}">
                                    <input type="radio" name="correct_answer" value="" class="mt-3" {{ old('correct_answer') === (old('choices.1') ?? '') ? 'checked' : '' }}>
                                    <span class="text-sm text-gray-600 mt-3">Benar</span>
                                </div>
                                <div class="flex gap-2">
                                    <input type="text" name="choices[]" class="flex-1 border border-gray-300 rounded-lg px-4 py-2"
                                           placeholder="Pilihan 3" value="{{ old('choices.2') }}">
                                    <input type="radio" name="correct_answer" value="" class="mt-3" {{ old('correct_answer') === (old('choices.2') ?? '') ? 'checked' : '' }}>
                                    <span class="text-sm text-gray-600 mt-3">Benar</span>
                                </div>
                                <div class="flex gap-2">
                                    <input type="text" name="choices[]" class="flex-1 border border-gray-300 rounded-lg px-4 py-2"
                                           placeholder="Pilihan 4" value="{{ old('choices.3') }}">
                                    <input type="radio" name="correct_answer" value="" class="mt-3" {{ old('correct_answer') === (old('choices.3') ?? '') ? 'checked' : '' }}>
                                    <span class="text-sm text-gray-600 mt-3">Benar</span>
                                </div>
                            </div>
                    ...
                    <div class="flex gap-3">
                        ...
                        <a href="{{ route('modules.edit', $chapter->module) }}" class="bg-gray-300 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-400">
                            Batal
                        </a>
                    </div>
                </form>

        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden mb-8 transform transition-all duration-500 hover:shadow-2xl">
            <div class="bg-gradient-to-r from-indigo-500 to-purple-600 p-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <a href="{{ route('guru.modul.chapter.pages.create', $chapter->id) }}"
            ...
            @forelse ($pages as $p)
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md hover:shadow-xl transform transition-all duration-300 hover:scale-[1.02] overflow-hidden">
                    <div class="p-6">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                            <div class="flex items-center mb-4 sm:mb-0">
                                <div class="flex-shrink-0 bg-gradient-to-br {{ $p->type === 'video' ? 'from-red-400 to-pink-500' : 'from-blue-400 to-indigo-500' }} rounded-lg p-3 mr-4">
                                    @if($p->type === 'video')
                                ...
                                <div>
                                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Halaman {{ $p->page_number }}</h3>
                                    <div class="flex items-center mt-1">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $p->type === 'video' ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' : 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200' }}">
                                            {{ ucfirst($p->type) }}
                                        </span>
                                        @if($p->type === 'video' && $p->video_url)
                                            <span class="ml-2 text-xs text-gray-500 dark:text-gray-400 truncate max-w-xs">{{ $p->video_url }}</span>
                                        @elseif($p->type === 'question' && $p->question_text)
                                            <span class="ml-2 text-xs text-gray-500 dark:text-gray-400 truncate max-w-xs">{{ Str::limit($p->question_text, 50) }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            <div class="flex space-x-3">
                                <!-- Edit Button -->
                                <a href="{{ route('guru.modul.chapter.pages.edit', [$chapter->id, $p->id]) }}"
                                ...
                                <!-- Delete Button -->
                                <form action="{{ route('guru.modul.chapter.pages.delete', [$chapter->id, $p->id]) }}"
                                      method="POST"
